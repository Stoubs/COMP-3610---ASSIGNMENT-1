...
codes_sql = (
    "(" + ",".join(str(c) for c in selected_codes) + ")"
...
with tab1:
    ...
    if len(df_r) > 0 and metrics.total_trips > 0:
        ...
        st.write(
            f"**Insight:** The busiest pickup zone is **{top_zone}** with **{top_count:,} trips** "
    ...
    fig_s.update_traces(
        line=dict(color="#FF4500", width=3), marker=dict(color="#FF4500")
    )
    ...
    if len(df_s) > 0:
        ...
        st.write(
            f"**Insight:** The highest average fare occurs around **{int(peak.pickup_hour)}:00** "
    ...
    if len(df_t) > 0:
        ...
        st.write(
            f"**Insight:** The median trip distance is **{median_dist} miles**, and about **{short_share}%** "
    ...
    if len(df_u) > 0:
        df_u_sorted = df_u.sort_values("trip_count", ascending=False).reset_index(
            drop=True
        )
    ...
    if len(df_v) > 0:
        ...
        st.write(
            f"**Insight:** The busiest time in the filtered data is **{peak_cell.day_of_week}** around "
